...
const PenaltyPayment = () => {
  ...
  const [penaltyInfoMap, setPenaltyInfoMap] = useState({});
  ...
  const fetchPenaltyForSelectedTickets = async (userId, ticketSelections) => {
    if (!userId || !Array.isArray(ticketSelections) || ticketSelections.length === 0) {
      setPenaltyInfoMap({});
      return;
    }

    // Only process chit-type selections
    const chitSelections = ticketSelections.filter((sel) =>
      typeof sel === "string" && sel.startsWith("chit-")
    );

    const newPenaltyMap = {};

    try {
      // Group by group_id to minimize API calls
      const groupToTickets = {};
      for (const sel of chitSelections) {
        const [_, entity] = sel.split("-");
        const [groupId, ticket] = entity.split("|");
        if (!groupToTickets[groupId]) groupToTickets[groupId] = [];
        groupToTickets[groupId].push({ selectionKey: sel, ticket });
      }

      // Fetch penalty report per group
      for (const [groupId, tickets] of Object.entries(groupToTickets)) {
        const res = await api.get("/penalty/get-penalty-report", {
          params: { user_id: userId, group_id: groupId },
        });

        const penaltyDataList = res.data?.data || [];

        for (const { selectionKey, ticket } of tickets) {
          const match = penaltyDataList.find(
            (item) =>
              String(item.user_id) === String(userId) &&
              String(item.group_id) === String(groupId) &&
              String(item.ticket_number) === String(ticket)
          );

          if (match?.summary) {
            const summary = match.summary;
            const regularPenalty = Number(summary.total_penalty || 0) - Number(summary.total_vacant_chit_penalty || 0);
            const vcPenalty = Number(summary.total_vacant_chit_penalty || 0);
            const lateFee = Number(summary.total_late_payment_charges || 0);
            const totalPenalty = regularPenalty + vcPenalty;

            newPenaltyMap[selectionKey] = {
              dueAmount: Number(summary.total_due) || 0,
              regularPenalty,
              vcPenalty,
              lateFee,
              penalty: totalPenalty, // regular + VC
              totalCharges: totalPenalty + lateFee,
              groupName: match.group_name || "Unknown Group",
              ticket,
            };
          } else {
            newPenaltyMap[selectionKey] = null;
          }
        }
      }

      setPenaltyInfoMap(newPenaltyMap);
    } catch (err) {
      console.error("Failed to fetch penalties for selected tickets:", err);
      setPenaltyInfoMap({});
    }
  };
  ...
  const handlePaymentAntSelect = async (values) => {
    setPaymentGroupTickets(values);
    setErrors((prev) => ({ ...prev, payment_group_tickets: "" }));

    if (selectedUserId) {
      await fetchPenaltyForSelectedTickets(selectedUserId, values);
    } else {
      setPenaltyInfoMap({});
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isValid) {
        ...
        await api.post("/payment/add-payments-auto-penalty", formData);
                    ...
                    >
                      ...
                      {paymentGroupTickets.length > 0 && (
                        <div className="mt-6 bg-white border border-gray-200 rounded-lg shadow-md">
                          ...
                          <div className="p-6">
                            {paymentGroupTickets.map((selection, index) => {
                              const info = penaltyInfoMap[selection];
                              if (!info) return null;

                              const [_, entity] = selection.split("-");
                              const [groupId, ticket] = entity.split("|");
                              const displayName = `${info.groupName} | ${ticket}`;

                              return (
                                <div key={selection} className={index < paymentGroupTickets.length - 1 ? 'mb-6 pb-6 border-b border-gray-100' : 'mb-6'}>
                                  ...
                                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div className="bg-gray-50 rounded p-3">
                                      <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Due Amount</p>
                                      <p className="text-lg font-semibold text-gray-900">
                                        ₹{Number(info.dueAmount).toLocaleString("en-IN")}
                                      </p>
                                    </div>
                                    ...
                                    <div className="bg-gray-50 rounded p-3">
                                      ...
                                      <p className="text-lg font-semibold text-gray-900">
                                        ₹{Number(info.penalty).toLocaleString("en-IN")}
                                      </p>
                                    </div>
                                    ...
                                    <div className="bg-gray-50 rounded p-3">
                                      ...
                                      <p className="text-lg font-semibold text-gray-900">
                                        ₹{Number(info.lateFee).toLocaleString("en-IN")}
                                      </p>
                                    </div>
                                  </div>
                                </div>
                            ...
                            <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                              <div className="flex items-center justify-between">
                                <h4 className="text-base font-medium text-gray-900">Total Penalty Summary</h4>
                                <div className="flex items-center gap-6">
                                  <div className="text-right">
                                    <p className="text-xs text-gray-500 uppercase tracking-wide">Total Penalty</p>
                                    <p className="text-lg font-semibold text-gray-900">
                                      ₹{paymentGroupTickets.reduce((sum, selection) => {
                                        const info = penaltyInfoMap[selection];
                                        return sum + (info ? Number(info.penalty) : 0);
                                  ...
                                  <div className="text-right">
                                    ...
                                    <p className="text-lg font-semibold text-gray-900">
                                      ₹{paymentGroupTickets.reduce((sum, selection) => {
                                        const info = penaltyInfoMap[selection];
                                        return sum + (info ? Number(info.totalCharges) : 0);
                                      }, 0).toLocaleString("en-IN")}
                                    </p>
                                  </div>
                                </div>
